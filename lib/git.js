...
exports.ensureBranch = function ensureBranch(name) {
  try {
    execSync(
      fmt('git show-ref --heads --abbrev "%s"', name),
      fmt('Failed to find branch "%s", will try creating', name),
      'ignore output'
    );
  } catch (e) {
    execSync(
...
exports.syncBranch = function gitSyncBranch(gitDstBranch) {
  ...
  // Get ref name of source branch, to validate it exists, and because
  // we will use it later.
  var srcRefInfo = execSync(
    fmt('git show-ref --heads "%s"', gitSrcBranch),
    fmt('Failed to parse branch `%s`, does it exist?', gitSrcBranch)
  );
  var srcSha = srcRefInfo.split(' ')[0];
  var srcRef = srcRefInfo.split(' ')[1];

  // Get ref name of destination branch, to validate it exists, and because
  // we will use it later.
  var dstRefInfo = execSync(
    fmt('git show-ref --heads "%s"', gitDstBranch),
    fmt('Failed to parse branch `%s`, does it exist?', gitDstBranch)
  );
  var dstSha = dstRefInfo.split(' ')[0];
  var dstRef = dstRefInfo.split(' ')[1];

  var mergeBaseCmd = fmt('git merge-base "%s" "%s"', srcSha, dstSha);
  ...
  var commitSha = execSync(
    fmt('git commit-tree -p "%s" -p "%s" -m "%s" %s',
        dstRef, srcRef, message, gitSrcTreeSha),
...
exports.commitAll = function gitCommitAll(gitDstBranch) {
  ...
  diff = shell.exec(fmt('git diff --quiet "%s" "%s"', commitSha, dstRef),
                    { silent: true });
  if (diff.code === 0)
    return {};
